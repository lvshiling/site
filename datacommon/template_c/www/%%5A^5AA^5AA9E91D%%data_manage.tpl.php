<?php /* Smarty version 2.6.19, created on 2011-04-02 17:50:18
         compiled from user/data_manage.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'html_options', 'user/data_manage.tpl', 4, false),)), $this); ?>
<div class="tcat" style="padding:2px;">
<select class="formInput" name="operate" onchange="executeOperate();">
<option value="">批量管理选项</option>
<?php echo smarty_function_html_options(array('options' => $this->_tpl_vars['ManageSelect']), $this);?>
...
<tr><td class="text_right" width="180">管理操作：</td><td><input type="radio" name="upgrade" value="1" checked="checked" />提升资源&nbsp;<input type="radio" name="upgrade" value="0" />下沉资源<br /><span class="form_clue">提升资源的种子数不少于<?php echo $this->_tpl_vars['Config']['upgrade_least_seeder']; ?>
且两次提升间隔时间不小于<?php echo $this->_tpl_vars['Config']['upgrade_space_time']; ?>
小时</span></td></tr>
</table>
...
function executeOperate()
{
	...
	if ('' == s_type)
	{
		$('manage_area').setStyle('display', 'none');
		return false;
	}
	...
	{
		$('manage_area').setStyle('display', '');
		for (var i = 0; i < manage_option.length; i++)
		{
			if (manage_option[i] == s_type)
			{
				$('manage_'+ manage_option[i]).setStyle('display', '');
			}
			else
			{
				$('manage_'+ manage_option[i]).setStyle('display', 'none');
			}
		}
	}
}
